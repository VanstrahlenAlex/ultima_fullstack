	...
	return (
		<Collapsible.Root open={open} onOpenChange={setOpen} className="mb-4">
			...
			<Collapsible.Content forceMount>
				...
				>
					{items.map((item) =>(
						<Button className="w-full justify-start text-foreground/80 font-normal" variant="link" key={item.href} asChild>
							<Link className={`flex items-center rounded-md px-5 py-1 transition-all ${pathname === item.href ?
								"bg-foreground/10 hover:bg-foreground/5" : "hover:bg-foreground/5"
							}`} href={item.href}>
								{item.icon}
				...
				<DropdownMenu>
					...
					<DropdownMenuContent align="end" className="w-56">
						...
						<DropdownMenuItem onClick={() => {
							//logout
						}} className="text-sm hover:text-primary" variant="destructive">
